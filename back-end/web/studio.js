...
<div class="result-grid">
 <form id="imgform" class="card">
  <label class="upload">Ảnh sản phẩm của anh <span class="hint">· JPG/PNG/WEBP dưới 20 MB</span><input type="file" id="imgfile" accept="image/png,image/jpeg,image/webp"><img id="imgprev" ${img.dataUrl?`src="${esc(img.dataUrl)}"`:'hidden'} alt="Ảnh sản phẩm"><button class="text-button" type="button" data-action="img-remove" ${img.dataUrl?'':'hidden'}>Bỏ ảnh</button></label>
...
function brief(){
 ...
 <label class="upload">Thêm ảnh <span class="hint">· không bắt buộc, tối đa 20 MB</span><input type="file" id="asset" accept="image/png,image/jpeg,image/webp"><img id="asset-preview" ${current.image?`src="${esc(current.image)}"`:'hidden'} alt="Ảnh của anh"><button class="text-button" type="button" data-action="remove-image" ${current.image?'':'hidden'}>Bỏ ảnh</button></label>
