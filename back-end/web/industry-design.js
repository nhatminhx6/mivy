...
function drawBadgePill(ctx, text, x, y, bg, border, color, dotColor = null, customH = 38) {
  ctx.save();
  const h = customH;
...
function drawFullPhotoRecruitmentHero(canvas, copy, asset, th, bgImg = null) {
  ...
  const bentoH = isCompact ? 100 : 120;
  const colGap = 16;
  ...
  const bentoCards = [
    ...
    },
    {
      icon: '⚙️',
      tag: 'KINH NGHIỆM',
      val: expFact.length > 22 ? expFact.slice(0, 20) + '…' : expFact,
      highlight: false,
    },
    {
      icon: '📍',
      tag: 'ĐỊA ĐIỂM & CHẾ ĐỘ',
      val: locFact.length > 22 ? locFact.slice(0, 20) + '…' : locFact,
      highlight: false,
    },
  ];
...
function drawFullPhotoRecruitmentStory(canvas, copy, asset, th, bgImg = null) {
  const w = 1080, h = canvas.height;
  const ctx = canvas.getContext('2d');
...
function drawFullPhotoRecruitmentAction(canvas, copy, asset, th, bgImg = null) {
  const w = 1080, h = canvas.height;
  const ctx = canvas.getContext('2d');
...
function drawFullPhotoGeneral(canvas, kind, copy, asset, th, bgImg = null) {
  const w = 1080, h = canvas.height;
  const ctx = canvas.getContext('2d');
...
function drawIndustryPoster(canvas, kind, raw, asset, bgImg = null) {
  ...
  if (isFullPhoto) {
    if (marketing.industry === 'recruitment') {
      if (kind === 'launch') {
        drawFullPhotoRecruitmentHero(canvas, copy, asset, th, bgImg);
      } else if (kind === 'story') {
        drawFullPhotoRecruitmentStory(canvas, copy, asset, th, bgImg);
      } else {
        drawFullPhotoRecruitmentAction(canvas, copy, asset, th, bgImg);
      }
      return;
    }
    drawFullPhotoGeneral(canvas, kind, copy, asset, th, bgImg);
    return;
  }
